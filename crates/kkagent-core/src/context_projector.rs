...
fn project_owned_impl(
    ...
    // Todo tool calls/results are harness state replays, not conversation
    // content: fold them in the non-recent prefix regardless of folding mode.
    if fold_todo_history(&mut messages, keep) > 0 {
        // Folded messages must not break provider caches mid-flight is
        // accepted here: Todo snapshots are exactly the payload growth this
...
/// Compact representation used for historical Todo tool calls/results.
const TODO_WRITE_RESULT_STUB: &str = "Todo state updated.";

/// True when a TodoList tool call performs a full-state read (`list` op) or a
/// full-list write (legacy `todos: [...]` shape). Those are the payloads we
/// compact away in projected history.
fn is_todo_tool_name(name: &str) -> bool {
    name == "TodoList"
}

fn todo_tool_use_is_read(input: &serde_json::Value) -> bool {
    if input.get("ops").is_some() {
        // Reads are explicit `list` ops inside a batch; a batch that also
        // mutates is still a write whose args are small. Treat a batch
        // containing only `list` ops as a read.
        input
            .get("ops")
        ...
        // Legacy shape: `todos: [...]` write or plain read (no args).
        input.get("todos").is_none()
    }
}

fn todo_tool_use_arg_chars(input: &serde_json::Value) -> usize {
...
mod todo_fold_tests {
    ...
    /// Simulate the real next-turn message construction: a Todo write call
    /// (assistant ToolUse), its compact result, then filler conversation and
    /// the next user message.
    #[test]
    fn projected_context_drops_historical_todo_full_snapshots() {
        let mut messages = Vec::new();
        // Legacy full-list write payload (the worst historical offender).
        let full_list: Vec<serde_json::Value> = (1..=20)
            .map(|i| serde_json::json!({ "title": format!("task {i}"), "status": "pending" }))
            .collect();
        messages.push(ChatMessage {
            role: "assistant".into(),
            content: vec![ChatContent::ToolUse {
                id: "todo-write-1".into(),
                name: "TodoList".into(),
                input: serde_json::json!({ "todos": full_list }),
            }],
            tools: None,
        });
        ...
        // Explicit list read echo.
        messages.push(ChatMessage {
            role: "assistant".into(),
            content: vec![ChatContent::ToolUse {
                id: "todo-list-1".into(),
                name: "TodoList".into(),
                input: serde_json::json!({ "ops": [{ "op": "list" }] }),
            }],
            tools: None,
        });
        messages.push(tool_result(
            "todo-list-1",
            "Current todo list:\n  [in_progress] task 1 (todo-1)\n…20 items…",
        ));
        // Filler so the Todo messages fall outside keep_recent.
        for i in 0..16 {
            messages.push(user_text(&format!("filler {i}")));
        }
        messages.push(user_text("next user message"));
        ...
        for msg in &projected {
            for part in &msg.content {
                match part {
                    ChatContent::Text { text } => serialized.push_str(text),
                    ChatContent::ToolUse { input, .. } => {
                        serialized.push_str(&serde_json::to_string(input).unwrap());
                    }
                    ChatContent::ToolResult { content, .. } => serialized.push_str(content),
                    _ => {}
                }
            }
        }
        assert!(
            !serialized.contains("task 20"),
            "projected context must not rescan full todo items: {serialized}"
        );
        assert!(
            !serialized.contains("Current todo list:"),
            "historical full todo renderings must be folded"
        );
        // Protocol validity: pairing survives.
        let has_use = projected.iter().any(|m| {
            m.content
                .iter()
                .any(|c| matches!(c, ChatContent::ToolUse { id, .. } if id == "todo-write-1"))
        });
        let has_result = projected.iter().any(|m| {
            m.content
                .iter()
                .any(|c| matches!(c, ChatContent::ToolResult { tool_use_id, .. } if tool_use_id == "todo-write-1"))
        });
        assert_eq!(has_use, has_result, "tool pairing must stay valid");
    }

    /// The compact write result (transition summary) is preserved when it is
    /// inside the recent window: the model keeps current-task focus.
    #[test]
    fn recent_todo_write_summary_stays_visible() {
        let mut messages = Vec::new();
        messages.push(ChatMessage {
            role: "assistant".into(),
            content: vec![ChatContent::ToolUse {
                id: "todo-w".into(),
                name: "TodoList".into(),
                input: serde_json::json!({ "ops": [{ "op": "complete", "id": "todo-1" }] }),
            }],
            tools: None,
        });
        messages.push(tool_result(
            "todo-w",
            "Task completed: task 1. Current task: task 2. Progress: 1/20 completed.",
        ));
        messages.push(user_text("continue"));

        let projected = project(&messages, &ProjectOptions::default());
        let mut all = String::new();
        for msg in &projected {
            for part in &msg.content {
                if let ChatContent::Text { text } | ChatContent::ToolResult { content: text, .. } =
                    part
                {
                    all.push_str(text);
                }
        ...
        assert!(all.contains("Current task: task 2."));
        assert!(all.contains("Progress: 1/20 completed."));
    }
}
